<template>
<section>
    <organBtnGroup :currentStep="3" @confirm="search" />
...
import {getOrgShlist} from '@/service/getData'
...
export default {
    ...
    methods : {
        async initData(page,size){
            this.loading = true;
            let res = await getOrgShlist(this.account,this.orgName,this.status,page,size);
            if(res && res.code == 200){
